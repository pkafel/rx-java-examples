...
public class RetrofitExample {
    ...
    public UserProfile getUserProfileExample(UUID userId) {
        ...
        final Observable<UserContacts> userContacts = contactClient.getContactsByUserId(userId)  // get user's contacts
                ...
                .mergeMap( contact -> userClient.getUser(contact.getUserId() ),                  // for each contact call user client in order to get details
                          (jsonHolder, jsonElement) -> new UserContact(jsonHolder, jsonElement)) // and zip contact with user data (or do with it whatever you like)
        ...
        return Observable.zip(userData, userContacts, (userDataToZip, userContactsToZip) -> new UserProfile(userDataToZip, userContactsToZip))
